...
def alert(kind: str, message: str) -> None:
    ...
    try:
        ...
        logger.error(
            "no se pudo enviar la alerta al webhook de Handoff: HTTP %s", exc.response.status_code
        )
    except Exception as exc:  # noqa: BLE001 - la alerta no puede fallar
        # The webhook URL is the credential; it must never reach the logs.
        logger.error("no se pudo enviar la alerta al webhook de Handoff: %s", type(exc).__name__)
