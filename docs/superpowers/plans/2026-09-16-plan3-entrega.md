...
Nota de una tercera ronda de revisión: la cabecera se quedó fuera de las dos
rondas anteriores. `empleados_aprox` se interpolaba sin pasar por
`_escape_mrkdwn` (un `"50<!channel>"` producía un `<!channel>` vivo), y ni
`nombre`, `cargo`, `empresa.nombre` ni `empleados_aprox` tenían tope: un
`persona.nombre` de 5000 caracteres producía una sección de más de 5000. Y
`puntuacion` y `vacantes_abiertas` se interpolaban igual de crudos, sin
garantía de que fueran realmente números. La solución no es escapar y capar
la cabecera nada más: es que ningún campo numérico (`empleados_aprox`,
`puntuacion`, `vacantes_abiertas`) se renderice salvo que sea un `int` de
verdad (`_safe_number`, que descarta `bool` y strings numéricos como si el
campo faltara), y que `nombre`, `cargo` y `empresa.nombre` pasen por
`_escape_and_cap` igual que cualquier otro campo de texto de terceros.
